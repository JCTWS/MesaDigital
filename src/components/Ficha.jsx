...
export default function FichaSacramento({ mesaLink, onVidaSnapshot } = {}) {
  ...
  // IMPORTANTE: este hook fica ANTES do "if (loading) return" abaixo —
  // hooks nunca podem vir depois de um retorno condicional.
  useEffect(() => {
    if (!onVidaSnapshot) return;
    const vMax = 6 + clamp0(character.atributos.fisico) + clamp0(character.vidaBonusManual);
    onVidaSnapshot({ nome: character.nome, vidaMax: vMax, vidaPerdida: character.vidaPerdida, dorMax: 6, dorMarcada: character.dorMarcada });
  }, [character, onVidaSnapshot]);
